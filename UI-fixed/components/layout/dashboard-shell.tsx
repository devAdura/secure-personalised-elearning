  ...
  return (
    <div className="min-h-screen bg-[hsl(var(--background))]">
      <aside className="fixed inset-y-0 left-0 hidden w-72 border-r border-white/70 bg-white/80 shadow-[0_24px_80px_rgba(18,32,28,0.08)] backdrop-blur-xl lg:block">
        ...
        <div className="p-4">
          <div className="mb-5 rounded-lg border border-emerald-900/10 bg-[#eef8f1] p-4">
          ...
          <nav className="space-y-1">
            {roleLinks[user.role].map(({ href, label, icon: Icon }) => (
              <Link
                ...
                href={href}
                className={cn(
                  "group flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-bold text-[#53625b] transition hover:bg-white hover:text-[#12201c] hover:shadow-sm"
                )}
              >
      ...
      <div className="lg:pl-72">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-white/70 bg-white/90 px-4 backdrop-blur-xl sm:px-6">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.18em] text-[#176b58]">SecureLearn command center</p>
            <p className="mt-1 text-sm text-[#68766e]">Biometric assurance, collaboration, and adaptive learning.</p>
          </div>
