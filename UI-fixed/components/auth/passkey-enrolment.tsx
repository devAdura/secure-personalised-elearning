...
export function PasskeyEnrolment({ existingCount }: { existingCount: number }) {
  ...
  return <div className="space-y-5">{message ? <Alert variant={message.type === "info" ? "info" : message.type}>{message.text}</Alert> : null}<div className="premium-card-subtle p-5"><div className="flex items-start gap-4"><div className="rounded-lg bg-emerald-100 p-3 text-emerald-700"><ShieldCheck className="h-6 w-6" /></div><div><h3 className="font-black text-[#12201c]">Privacy-preserving biometric security</h3><p className="mt-1 text-sm leading-6 text-muted-foreground">The fingerprint scan remains inside your device authenticator. SecureLearn receives a signed cryptographic response and stores only the public credential data needed to verify future logins.</p></div></div></div><div className="flex items-center justify-between rounded-lg border border-[#d8e5de] bg-white/90 p-4 shadow-sm"><div><p className="font-black text-[#12201c]">Registered passkeys</p><p className="text-sm text-muted-foreground">{existingCount} credential{existingCount === 1 ? "" : "s"} connected</p></div><Fingerprint className="h-8 w-8 text-primary" /></div><Button size="lg" className="w-full" disabled={loading} onClick={enrol}><Fingerprint className="h-5 w-5" />{loading ? "Follow your device prompt..." : "Enrol fingerprint/passkey"}</Button></div>;
}
