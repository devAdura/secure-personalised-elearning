...
export function CourseForm({ initial = {} }: { initial?: InitialCourse }) {
  ...
  return <form onSubmit={submit} className="space-y-5">{error ? <Alert variant="error">{error}</Alert> : null}<div className="space-y-2"><Label htmlFor="title">Course title</Label><Input id="title" name="title" defaultValue={initial.title} required /></div><div className="space-y-2"><Label htmlFor="description">Description</Label><Textarea id="description" name="description" defaultValue={initial.description} required className="min-h-36" /></div><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="category">Category</Label><Input id="category" name="category" defaultValue={initial.category} placeholder="Computer Science" required /></div><div className="space-y-2"><Label htmlFor="level">Level</Label><select id="level" name="level" defaultValue={initial.level || "Beginner"} className="select-control"><option>Beginner</option><option>Intermediate</option><option>Advanced</option></select></div></div><div className="space-y-2"><Label htmlFor="thumbnailUrl">Thumbnail URL (optional)</Label><Input id="thumbnailUrl" name="thumbnailUrl" type="url" defaultValue={initial.thumbnailUrl || ""} placeholder="https://..." /></div><label className="flex items-center gap-3 rounded-lg border border-[#d8e5de] bg-white/80 p-4 shadow-sm"><input type="checkbox" name="isPublished" defaultChecked={initial.isPublished} className="h-4 w-4 accent-primary" /><span><span className="block text-sm font-black text-[#12201c]">Publish this course</span><span className="text-xs leading-5 text-muted-foreground">Published courses are visible in the student course catalogue.</span></span></label><SubmitButton loading={loading}>{initial.id ? "Update course" : "Create course"}</SubmitButton></form>;
}
