...
export default async function CoursesPage({ searchParams }: { searchParams: Promise<{ q?: string; category?: string; level?: string }> }) {
  ...
  return <><SiteHeader/><main className="min-h-[70vh]"><section className="route-hero"><div className="page-container route-hero-grid"><div className="route-copy"><p className="kicker">Course catalogue</p><h1>Find the right secured learning path.</h1><p>Browse published courses, compare learning levels, then sign in as a student to enrol, access materials, submit assignments, and join course discussions.</p>{isDemoMode?<div className="mt-5 max-w-3xl"><Alert><strong>Limited catalogue mode.</strong> Sample courses are available while live course services recover. Enrolment and course activity may be temporarily unavailable.</Alert></div>:null}</div><aside className="hero-terminal premium-card"><div className="terminal-row"><span>Catalogue state</span><strong>{isDemoMode?"Limited preview":"Available"}</strong></div><div className="terminal-row"><span>Visible courses</span><strong>{courses.length}</strong></div><div className="terminal-row"><span>Filters active</span><strong>{[q,category,level].filter(Boolean).length}</strong></div><div className="absolute bottom-4 left-4 right-4 z-10 grid gap-3 sm:grid-cols-3"><div className="rounded-lg border border-white/15 bg-white/10 p-3 backdrop-blur-md"><ShieldCheck className="h-5 w-5 text-[#b7f0dc]"/><p className="mt-2 text-xs font-bold text-white/75">Protected content</p></div><div className="rounded-lg border border-white/15 bg-white/10 p-3 backdrop-blur-md"><Users className="h-5 w-5 text-[#f5cb78]"/><p className="mt-2 text-xs font-bold text-white/75">Collaborative flow</p></div><div className="rounded-lg border border-white/15 bg-white/10 p-3 backdrop-blur-md"><Database className="h-5 w-5 text-white"/><p className="mt-2 text-xs font-bold text-white/75">Current records</p></div></div></aside></div></section><section className="page-container pt-8"><form className="filter-shell w-full" aria-label="Filter courses"><label className="input-shell"><Search/><input name="q" defaultValue={q} className="select-control pl-9" placeholder="Search courses" aria-label="Search courses"/></label><select name="category" defaultValue={category} className="select-control" aria-label="Course category"><option value="">All categories</option>{categories.map((c)=><option key={c.category} value={c.category}>{c.category}</option>)}</select><select name="level" defaultValue={level} className="select-control" aria-label="Course level"><option value="">All levels</option><option>Beginner</option><option>Intermediate</option><option>Advanced</option></select><button className={buttonVariants()}>Apply filters</button></form></section><section className="page-container py-10">{courses.length?<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{courses.map((course)=><CourseCard key={course.id} course={course}/>)}</div>:<EmptyState icon={BookOpen} title="No matching courses" description="Try a broader keyword or remove one of the filters."/>}</section></main><SiteFooter/></>;
}
